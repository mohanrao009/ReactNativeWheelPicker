 ...
 * */
 ...
 export default class TimePicker extends React.Component<Props, State> {
   ...
   render() {
     ...
     return (
       <View style={[styles.container, { backgroundColor: this.props.backgroundColor }]}>
         ...
         <Text style={{alignItems:'center',textAlign:'center',marginTop:15, fontSize:30,color:'#000', justifyContent:'center'}}>:</Text>
         <WheelPicker
           isCyclic
           style={styles.wheelPicker}
           {...this.props}
   ...
   onHourSelected = (position: number) => {
     ...
     this.onTimeSelected(Number(this.state.hours[position]) ,Number(this.state.minutes[this.state.selectedMinuteIndex]))
   }
   ...
   onMinuteSelected = (position: number) => {
     ...
     this.onTimeSelected(Number(this.state.hours[this.state.selectedHourIndex]) ,Number(this.state.minutes[position]))
   }
   ...
   onAmSelected = (position: number) => {
     ...
     const selectedHour24format = hourTo24Format(newTime12Format)
     selectedDate.setHours(selectedHour24format)
     // this.onTimeSelected(position)
   }
 
   onTimeSelected(hours: Number, minutes: Number) {
     if (this.props.onTimeSelected) {
       this.props.onTimeSelected(hours, minutes)
     }
   }
 }
